...
import { SILO_GRAM_CONVERSIONS } from '../constants';
import { get } from 'lodash';
type ResponseType = {
  readAll: number[];
};
const endpoint = SERVER_URL.URI_SILOS;

export interface SilosInterface {
  readAll(): number[];
...
export class Silos {
  ...
  private baselineGramsConversion = [
    // @ts-ignore
    parseFloat(process.env.SILO_BASELINE_GRAMS_1),
    // @ts-ignore
    parseFloat(process.env.SILO_BASELINE_GRAMS_2),
    // @ts-ignore
    parseFloat(process.env.SILO_BASELINE_GRAMS_3),
  ];
  constructor() {
    this.gramConversions = SILO_GRAM_CONVERSIONS;
    process.on('SIGINT', () => {
